...
public class Quiz {
	public static void main(String[] args) {
		...
		System.out.printf("평균:%.2f\n",	
				//평균값을 구하는 것으로
				// 국어는 int그대로 + 영어는 String 임으로 int값으로 변경하여 + 수학은 double로 int와 사용이 가능 / 과목수 
		...
		for(int s: source) {							// for문을 이용하여, source 배열에 있는 값을 하나씩 출력
			if(s%2==0) {								// 조건을 사용하여 source 값 하나씩 2로 나눴을때 나머지가 0이면
				even.add(s);							// 짝수 배열에 저장
			}else {										// 아니면(나머지가 0이 아니면)
				odd.add(s);								// 홀수 배열에 저장
			}
		}
		...
		// 큰 값에서 작은 값으로 정렬하는 것.
		for(int i=0;i<source.length;i++) {				// for문을 이용하여 배열에 있는 값(i)을 하나씩 출력
			for(int j=i+1;j<source.length;j++) {		// 이중 for문을 이용하여 값(i)+1 한 배열을 출력 
				if(source[i]<source[j]) {				// i가 j보다 작으면
					int temp=source[i];					// 임시 저장소 temp에 i값을 넣고
					source[i]=source[j];				// j값을 i에 저장
					source[j]=temp;						// temp에 있는 값을 j로 저장하여 값을 스왑한다.
				}
			}
		}
		...
		System.out.println(birthDate+"년"						// 19+87+"년"+sNum[0]에 2,3(1 1) +"월" + sNum[0]에 4,6(3 0)+ "일" + "| 성별:"+ gender
